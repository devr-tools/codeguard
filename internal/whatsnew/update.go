...
// updateCheckDisabled reports whether the environment opts out of update checks.
// It is disabled on the explicit env var or when a CI environment is detected,
// so codeguard never makes surprise outbound calls in automated pipelines.
...
func (c *UpdateChecker) writeCache(entry cacheEntry) {
	...
	// Best effort: a failed cache write only means we re-check next time.
