...
var fixTemplates = map[string]string{
	...
	"quality.ai.semantic-test-adequacy":     "Strengthen the nearby tests so they prove the changed behavior with meaningful assertions, negative paths, and boundary coverage.\n\nBefore:\nfunc TestBuildUser(t *testing.T) {\n\tBuildUser()\n}\n\nAfter:\nfunc TestBuildUserReturnsErrorOnConflict(t *testing.T) {\n\terr := BuildUser(conflictingInput)\n\tif err == nil {\n\t\tt.Fatal(\"expected conflict error\")\n\t}\n\tif !strings.Contains(err.Error(), \"conflict\") {\n\t\tt.Fatalf(\"error = %v, want conflict details\", err)\n\t}\n}",
