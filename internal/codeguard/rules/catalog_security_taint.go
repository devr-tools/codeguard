...
var securityTaintCatalog = map[string]core.RuleMetadata{
	...
	"security.taint.cpp": {
		ID:               "security.taint.cpp",
		Section:          "Security",
		DefaultLevel:     "fail",
		ExecutionModel:   core.RuleExecutionModelLanguageAgnostic,
		LanguageCoverage: core.FixedRuleLanguageCoverage(core.RuleLanguageCPP),
		Title:            "C++ taint flow",
		Description:      "Fails when untrusted input from the environment, process arguments, standard input, or a recognized web request flows into a shell or process execution API. The finding message includes the source-to-sink chain.",
		HowToFix:         "Use a fixed executable and argument vector, or constrain the input with a strict allowlist before invoking the process API.",
	},
	"security.ssrf.cpp": {
		ID:               "security.ssrf.cpp",
		Section:          "Security",
		DefaultLevel:     "fail",
		ExecutionModel:   core.RuleExecutionModelLanguageAgnostic,
		LanguageCoverage: core.FixedRuleLanguageCoverage(core.RuleLanguageCPP),
		Title:            "C++ server-side request forgery",
		Description:      "Fails when untrusted input flows into an outbound URL or host in recognized libcurl, cpr, Boost.Asio, cpprestsdk, or Poco APIs.",
		HowToFix:         "Parse and validate destinations against a trusted-host allowlist and reject private, loopback, and link-local addresses before connecting.",
	},
}
