...
func appendAdditionalLanguageLineFindings(env support.Context, file string, lineNo int, line string) []core.Finding {
	switch {
	case isRustFile(file):
		return appendRustLineFindings(env, file, lineNo, line)
	case isJavaFile(file):
		return appendJavaLineFindings(env, file, lineNo, line)
	case isCSharpFile(file):
		return appendCSharpLineFindings(env, file, lineNo, line)
	case isRubyFile(file):
		return appendRubyLineFindings(env, file, lineNo, line)
	default:
		return nil
	}
}

func appendRustLineFindings(env support.Context, file string, lineNo int, line string) []core.Finding {
	switch {
	case rustTLSPattern.MatchString(line):
	...
	case rustShellPattern.MatchString(line):
