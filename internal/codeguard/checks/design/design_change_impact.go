...
func changeImpactFindings(env support.Context, graphs []targetModuleGraph) []core.Finding {
	if env.Mode != core.ScanModeDiff || !designToggleEnabled(env.Config.Checks.DesignRules.DetectHighImpactChanges) {
		return nil
	}
	...
	if len(entries) > 0 && env.PutArtifact != nil {
		env.PutArtifact(core.NewChangeImpactArtifact(env.BaseRef, entries))
	}
	return findings
}
