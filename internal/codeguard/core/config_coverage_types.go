...
type CoverageDeltaConfig struct {
	...
	MinChangedLineCoverage *int `json:"min_changed_line_coverage,omitempty" yaml:"min_changed_line_coverage,omitempty"`
	// FailUnder, when set, escalates findings to fail for files whose
	// changed-line coverage falls below this percentage.
	FailUnder *int `json:"fail_under,omitempty" yaml:"fail_under,omitempty"`
	...
	LanguageCommands map[string]CoverageCommandConfig `json:"language_commands,omitempty" yaml:"language_commands,omitempty"`
}
...
type TestQualityRulesConfig struct {
	...
	AssertionHelpers []string `json:"assertion_helpers,omitempty" yaml:"assertion_helpers,omitempty"`
}
