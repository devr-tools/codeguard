...
func TestLoadConfigRejectsLegibilityHistorySymlinkEscape(t *testing.T) {
	dir := t.TempDir()
	...
	if err := os.Symlink(outside, filepath.Join(cacheDir, "cache.legibility-history.json")); err != nil {
		t.Skipf("symlinks unsupported on this platform: %v", err)
	}
	...
	_, err := service.LoadConfigFile(path)
	if err == nil {
		t.Fatal("expected a derived legibility-history symlink escape to be rejected")
	}
	if !strings.Contains(err.Error(), "legibility_history") || !strings.Contains(err.Error(), "symlink") {
		t.Fatalf("expected a legibility-history symlink containment error, got %v", err)
	}
}
