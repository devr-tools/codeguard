...
func TestCICheckFailsWhenTypeScriptTestsLiveOutsideAllowedPaths(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, filepath.Join(dir, "src", "sample.test.ts"), "export const sample = 1;\n")

	cfg := codeguard.ExampleConfig()
	cfg.Name = "ci-typescript-test-location-fail"
...
func TestCICheckPassesWhenTypeScriptTestsLiveUnderAllowedPaths(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, filepath.Join(dir, "tests", "unit", "sample.spec.tsx"), "export const sample = 1;\n")

	cfg := codeguard.ExampleConfig()
	cfg.Name = "ci-typescript-test-location-pass"
	cfg.Targets = []codeguard.TargetConfig{{Name: "web", Path: dir, Language: "typescript"}}
	cfg.Checks.CI = true
	cfg.Checks.Quality = false
	cfg.Checks.Design = false
...
func TestCICheckFailsWhenJavaScriptTestsUseTestsDirectoryLayoutOutsideAllowedPaths(t *testing.T) {
	dir := t.TempDir()
	writeFile(t, filepath.Join(dir, "__tests__", "sample.js"), "export const sample = 1;\n")

	cfg := codeguard.ExampleConfig()
	cfg.Name = "ci-javascript-test-layout-fail"
