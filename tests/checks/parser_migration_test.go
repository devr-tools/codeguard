...
func findingsForRule(report codeguard.Report, ruleID string) []string {
...
func TestPythonQualityIgnoresFunctionsInsideStrings(t *testing.T) {
	...
	messages := findingsForRule(report, "quality.max-parameters")
	if len(messages) != 1 {
...
func TestSecurityIgnoresPrimitivesInCommentsAndStrings(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if messages := findingsForRule(report, tc.ruleID); len(messages) != 0 {
